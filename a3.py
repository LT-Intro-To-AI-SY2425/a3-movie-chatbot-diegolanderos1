...
def movie_by_other_movie_year(matches: List[str]) -> List[str]:
    """Finds movies released in the same year as the given movie

    Args:
        matches - a list of 1 string, just the movie title

    Returns:
        a list of movies released in the same year as the given movie
    """
    original = matches[0]
    result = []
    year = 0
    for movie in movie_db:
        if get_title(movie) == original:
            year = get_year(movie)
    for movie in movie_db:
        if year == get_year(movie):
            result.append(get_title(movie))
...
pa_list: List[Tuple[List[str], Callable[[List[str]], List[Any]]]] = [
    ...
    (str.split("what movies came out the same year as %"), movie_by_other_movie_year),
    (["bye"], bye_action),
]
...
if __name__ == "__main__":
    ...
    assert isinstance(movie_by_other_movie_year(["boyz n the hood"]), list), "movie_by_other_movie_year not returning a list"
    assert sorted(movie_by_other_movie_year(["boyz n the hood"])) == sorted(
        ["dead again"]
    ), "failed movie_by_other_movie_year"
